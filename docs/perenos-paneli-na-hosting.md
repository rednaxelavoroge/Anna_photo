...
### Как переезд прошёл на самом деле

Сделано 27.08.2026, панель работает: открывается по
`https://admin.annamanasaryan.com/`, вход по паролю проходит, сохранение
доходит до GitHub (проверено коммитом «Обновление с панели управления» в
ветке `cursor/namecheap-static-f40b`).

Пять мест, где `.com` повёл себя не так, как `.art`:

1. **Поддомена в cPanel не было вовсе**, а завести его мешала старая запись
   DNS: `admin.annamanasaryan.com` был `CNAME` на `cname.vercel-dns.com`, и
   cPanel отвечал «A DNS entry for the domain already exists». Порядок
   оказался обратным описанному выше: сначала удалить запись в **Zone
   Editor**, сразу за этим создать домен в **Domains**, и только потом
   приложение. Запись `A` на сервер cPanel создал сам.

2. **Доступов к хостингу в репозитории Anna_photo не было.** Секреты
   `FTP_HOST` / `FTP_USER` / `FTP_PASSWORD` есть в репозитории `.art`, но не
   здесь, а прочитать их оттуда нельзя — GitHub не показывает сохранённые
   секреты никому. Завели отдельную FTP-учётку `panel@annamanasaryan.com`
   с корнем в `public_html` (cPanel → **FTP Accounts**) и положили её в
   секреты этого репозитория.

   Коварство в том, что при незаданном `FTP_HOST` выкладка не падает, а
   **пропускает все шаги и показывает зелёную галочку**. Первый запуск занял
   восемь секунд и «успешно» не сделал ничего. Отсюда правило: смотреть не
   на итог запуска, а на то, отработали ли шаги сборки.

3. **Кнопку выкладки пришлось заводить через файл-вызов.** Кнопка
   показывается только с ветки по умолчанию, а сайт живёт в рабочей ветке.
   Класть в `main` полную копию процесса нельзя — копии разъезжаются, и
   однажды панель уедет старым процессом молча. Поэтому в `main` лежит
   короткий файл с `uses: ...@cursor/namecheap-static-f40b`, а сам процесс —
   один, в рабочей ветке. Проверено запуском: в отчёте видно `deploy / deploy`.

4. **Перебор папок пришлось делать строже.** FTP-учётка попадает сразу
   внутрь `public_html`, поэтому в список кандидатов добавлено `.`. Но
   «папка открылась» — недостаточное доказательство: корень соседнего сайта
   выглядит так же, а ниже работает `mirror --delete`. Теперь папка обязана
   предъявить приметы именно этого сайта — `index.html` и раздел
   `phototour`. В журнале выкладки это видно строкой
   «. — подходит: это корень annamanasaryan.com».

5. **Сертификат выписался сам, ручной выпуск не понадобился.** cPanel при
   создании домена написал «Free 1-year StandardSSL installation is in
   progress» и справился к моменту проверки. Раздел про `acme.sh` выше
   остаётся на случай, если в следующий раз так не повезёт.

Плюс одна поломка в коде, которую видно только на хостинге: перенаправление
с корня поддомена в `src/middleware.ts` собирало адрес из `request.nextUrl`,
а за Passenger это внутренний `localhost:порт`. На Vercel работало, здесь
увело бы в никуда. Теперь адрес берётся из заголовков `Host` и
`X-Forwarded-Proto` самого запроса.

### Что осталось руками

Ничего из настройки не осталось — панель работает. Осталось два долга по
безопасности, отложенных сознательно, чтобы не трогать только что
настроенное:

- **Заменить ключ GitHub.** Значение переменной `GITHUB_TOKEN` видно в
  cPanel открытым текстом, и в ходе работы оно попало на снимок экрана в
  переписку. Ключ даёт полный доступ ко всем репозиториям. Порядок:
  выпустить новый, вписать его в cPanel → **Setup Node.js App** вместо
  старого, нажать **RESTART**, проверить сохранение из панели, и только
  после этого удалить старый на GitHub → Settings → Developer settings.
- **Сменить пароль панели.** Он тоже попал на снимок. Меняется в
  переменной `ADMIN_PASSWORD` там же, плюс в переменных проекта на Vercel,
  чтобы оба входа открывались одинаково.

### Как обновлять панель дальше

GitHub → **Actions** → **Выложить панель на хостинг** → **Run workflow**.
Ветку в списке можно не выбирать: процесс всё равно соберёт
`cursor/namecheap-static-f40b`. Первая выкладка заняла пять минут, дальше
уезжает только изменившееся.

Vercel остаётся запасным входом и продолжает работать: `NEXT_PUBLIC_MEDIA_BASE`
там не задан, а при пустом значении пути остаются прежними.
